...
its own uncertainty, the binding one sets the pose's margin, and one knob -- `k_sigma` -- says
...
def test_blocked_fraction_rises_with_k_sigma():
    fracs = [_solve(k, 0.025)[0].blocked.numpy().mean() for k in (0.0, 1.0, 2.0, 3.0)]
    assert fracs == sorted(fracs), f"not monotone in k_sigma: {fracs}"
    assert fracs[0] < fracs[-1], "k_sigma must actually bite"
...
def test_margin_weight_charges_only_below_z_ref():
    """The graded penalty is hinged: comfortable poses pay nothing."""
    plain, _ = _solve(0.5, 0.025, margin_weight=0.0, z_ref=4.0)
    graded, _ = _solve(0.5, 0.025, margin_weight=1.0, z_ref=4.0)
    ...
    assert dt[free].max() == pytest.approx(0.0, abs=1e-6), "poses above z_ref must pay nothing"
    assert dt.max() > 0.0, "poses below z_ref must pay something"


def test_uncertainty_can_make_a_goal_unreachable_and_k_sigma_recovers_it():
    ...
    honest consequence of not knowing the terrain. Lowering `k_sigma` is how a caller trades
