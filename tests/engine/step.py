...
def selftest_rollout_kernel():
    ...
    def buffers():
        ...
        # current_wheel_omega, loads, turning, clearance, clear_soft, residual, twist
        return [
...
def selftest_mu_scale():
    ...
    def run(mu_value, scale):
        ...
        out = [
            ...
            wp.zeros((T, B), dtype=wp.vec3, device="cpu"),
            wp.zeros((T, B), dtype=wp.vec2, device="cpu"),
            wp.zeros((T, B), dtype=float, device="cpu"),  # clear
            wp.zeros((T, B), dtype=float, device="cpu"),  # clear_soft
            wp.zeros((T, B), dtype=float, device="cpu"),  # resid
            wp.zeros((T + 1, B), dtype=wp.vec3, device="cpu"),  # twist
        ]
        wp.launch(
            rollout_kernel,
            B,
...
def selftest_motor_lag():
    ...
    and current_wheel_omega must converge toward target_wheel_omega with the expected time constant.
    """
    wp.init()
    tau = 0.2  # [s] motor lag
    dt = 0.05  # [s] timestep
    ...
    clear_soft = wp.zeros((T, 1), dtype=float, device="cpu")
    ...
    ref = rollout_np.rollout_terrain(
        ...
        k=2.0,
        tau_motor=tau,
        init_wheel_omega=np.zeros(3),
    )
    d_xy = np.abs(controlled.numpy()[1:, 0, :2] - ref["pose2"][:, :2]).max()

    print(
        f"motor_lag  d_current_wheel_omega={d_oa:.2e}  d_xy_vs_oracle={d_xy:.2e}  "
