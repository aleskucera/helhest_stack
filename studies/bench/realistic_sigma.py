...
class BeliefModel:
    ...
    def rho_kk(self, off_dy: np.ndarray, off_dx: np.ndarray) -> np.ndarray:
        ...
        `off_dy`/`off_dx` are [K] (sphere, shared by every node) or [N, K] (cylinder, one row per
        node); the `...` batches over the leading N without changing the [K] case at all."""
...
def _clark_moments(
    ...
    `off`, if given, is the precomputed (off_dy, off_dx, off_cap, rho_kk) tuple. That hoist is
    valid ONLY for the yaw-invariant sphere (`run_seed` builds it once and reuses it for every
    plan) -- the cylinder table depends on THIS plan's own yaw, so it is built fresh here, every
    call, whenever `off` is None (a sphere-only cache the audit flagged and this disables)."""
    ny, nx = belief.shape
...
def run_seed(seed: int, device: str, model: BeliefModel,
    ...
    ny, nx = belief.shape
    rp = RobotParams()
    # the sphere table is yaw-invariant, so it is hoisted ONCE and reused for every plan below;
    # the cylinder table is not (it depends on each plan's own yaw), so `_clark_moments` builds
    # it fresh per plan when `off` is None -- see that function's docstring.
    if element == "sphere":
...
def main() -> None:
    ...
    for seed in range(args.seeds):
        rows.append(run_seed(seed, args.device, model, args.family, args.noise, args.element))
        if (seed + 1) % 10 == 0:
