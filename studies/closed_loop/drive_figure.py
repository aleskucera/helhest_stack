...
def render(d, out: str) -> None:
    ...
    vshow = np.where(seen, np.where(v >= cap * 0.9, -1.0, v), np.nan)

    panels = [
        ("height the belief settled on  [m]", mask(d["height"]), "terrain", None),
        ("ever measured", seen.astype(float), "Greys_r", (0, 1)),
        ("blocked  (fraction of headings)", mask(blocked.mean(2)), "Reds", (0, 1)),
        ("cost-to-go the MPPI followed  [m]", vshow, "viridis", "route"),
    ]
