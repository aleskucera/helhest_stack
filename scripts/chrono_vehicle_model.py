...
HALF_TRACK = 0.365
REAR_OFFSET = 0.75
YAW_INERTIA = 10.135  # [kg m^2] about the CoM, from the mass table
WHEELS = {
...
WHEEL_MASS = 5.0  # [kg] each
INERTIA_XX, INERTIA_YY = 8.0, 12.0  # [kg m^2] roll/pitch of the whole body
...
def build_robot(sys, mat):
    """Chassis + three wheels on revolute speed motors. Returns (chassis, wheels, motors)."""
    chassis = chrono.ChBodyAuxRef()
    chassis.SetMass(MASS - 3.0 * WHEEL_MASS)
    chassis.SetFrameCOMToRef(chrono.ChFramed(chrono.ChVector3d(*COM), chrono.QUNIT))
    chassis.SetInertiaXX(chrono.ChVector3d(INERTIA_XX, INERTIA_YY, YAW_INERTIA))
    ...
    wheels, motors = {}, {}
    # a wheel's inertia about its own axis; the spin-up transient is set by MOTOR_TAU, not by this
    i_axis = 0.5 * WHEEL_MASS * WHEEL_RADIUS**2
    i_diam = 0.25 * WHEEL_MASS * WHEEL_RADIUS**2
    for name, (wx, wy, wz) in WHEELS.items():
        w = chrono.ChBody()
        w.SetMass(WHEEL_MASS)
        w.SetInertiaXX(chrono.ChVector3d(i_diam, i_axis, i_diam))
        ...
        sys.Add(w)
        # revolute about the wheel's own +y, speed-controlled: the engine commands wheel SPEED too
        ...
        m.SetSpeedFunction(chrono.ChFunctionConst(0.0))
        sys.Add(m)
        wheels[name], motors[name] = w, m
...
def drive(terrain_kind: str, omega: tuple[float, float, float], t_end: float,
    ...
    chassis, wheels, motors = build_robot(sys, mat)
    ...
    for i in range(n):
        ...
        for k in cmd:
            ...
            motors[k].SetSpeedFunction(chrono.ChFunctionConst(realized[k]))
        if terrain_kind == "scm":
            terrain.Advance(dt)
