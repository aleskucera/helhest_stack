...
class WarpDriver:
    ...
    def step(self, target_wheel_omega):
        target_wheel_omega = np.asarray(target_wheel_omega, np.float32).reshape(1, 1, 3)
        controlled, derived, clear, resid = self.sim.rollout(target_wheel_omega, self.controlled)
