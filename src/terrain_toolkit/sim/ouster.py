...
def make_osdome_lidar(
    ...
    *,
    ...
    Nominal uniform hemisphere beams + the real min/max range and the
    range-dependent precision curve. `dropout` isn't a datasheet figure (OSDome
    returns are dense) — expose it as a small optional stand-in for the far /
    low-reflectivity returns that a reflectivity model would drop.
    """
    dirs = osdome_beam_directions(nominal_osdome_polar(channels), cols, facing=facing)
    return PrimitiveLidar(
        dirs,
        ground=ground,
        noise_std=OSDOME_RANGE_NOISE_BASE_M,
        range_noise_quad=OSDOME_RANGE_NOISE_QUAD_M,
        range_noise_max=OSDOME_RANGE_NOISE_MAX_M,
        dropout=dropout,
        min_range=OSDOME_MIN_RANGE_M,
        max_range=OSDOME_MAX_RANGE_M,
        device=device,
    )
