...
def _drift_scan(obs, known, x0, y0, cell, rx, ry, dx, dy, dyaw):
    ...
    o = np.zeros_like(obs); k = np.zeros_like(known)
    o[qr[ok], qc[ok]] = obs[ri[ok], ci[ok]]
    k[qr[ok], qc[ok]] = True
    return o, k


def navigate(world, device="cuda", K=8, dock_radius=1.5, n_theta=24, lat_coarsen=4, win_m=9.0,
             route_m=16.0, local_scans=0, drift=0.0, fov_deg=180.0, max_range=7.0, mount_height=0.4,
             max_frames=1500, B=4096, T=70):
    ...
    plan_sim = Simulator(dynamics.robot_params(), dynamics.planning_solver(), win_grid, B, T, device)
    ...
    ctg = CostToGo(route_grid, dynamics.robot_params(), dynamics.planning_solver(),
    ...
    mm = MultiScanMap(scene.ny, scene.nx)  # the GLOBAL routing map the robot accumulates (drift-prone)
    ...
    drift_x = drift_y = drift_yaw = 0.0  # accumulated SE(2) global-map drift (m, m, rad), random walk
    trail, snaps = [], []
    ...
    for f in range(max_frames):
        ...
        obs, known = lidar_scan(scene.H, scene.x0, scene.y0, cell, (rx, ry, yaw),
                                fov_deg=fov_deg, max_range=max_range, mount_height=mount_height)
    ...
    return dict(reached=reached, frames=f + 1, closest=closest, contacts=contacts,
                known_frac=float(mm.known.mean()), scene=scene, goal=goal, snaps=snaps,
                win_world=win_world)
...
def _viz(world, res, out):
    ...
    for ax, (fr, known, trail, rxy) in zip(axes.ravel(), snaps):
        ...
        ax.add_patch(Rectangle((rxy[0] - wwm / 2, rxy[1] - wwm / 2), wwm, wwm,
                               fill=False, ec="cyan", lw=1.5, zorder=6))  # the planner window
    ...
    fig.suptitle(f"{world}: navigate on the lidar map, planned over a {wwm:.0f} m robot window  --  "
                 f"reached={res['reached']}, frames={res['frames']}, contacts={res['contacts']}"
                 f"   (cyan = the fixed planner window scrolling with the robot)", fontsize=12)
    fig.tight_layout()
...
def main():
    ...
    ap.add_argument("--local-scans", type=int, default=0,
    ...
    args = ap.parse_args()
    wp.init()
    res = navigate(args.world, device=args.device, K=args.K, dock_radius=args.dock_radius,
                   lat_coarsen=args.lat_coarsen, win_m=args.win_m, route_m=args.route_m,
                   local_scans=args.local_scans, drift=args.drift,
                   fov_deg=args.fov_deg, max_range=args.max_range)
    print(f"{args.world}: reached={res['reached']} frames={res['frames']} "
